...
- **Kiosk-Parameter (WebApp v0.0.131, Grafana 13):** Seit Grafana 13 akzeptiert die URL-Auswertung (`setKioskModeFromUrl`) NUR noch `kiosk=1` (→ Vollbild-Kiosk, `chromeless`: Sidebar UND obere Leiste/Zeitleiste weg). Die früheren Werte `kiosk=tv`/`kiosk`/`kiosk=full` sind im Nav-Umbau entfernt und werden **still ignoriert** (Dashboard erscheint komplett). Seit v0.0.131 setzen alle WebApp-Links + Global-Datalink `&kiosk=1`. „Powered by Grafana"-Badge unten rechts: fester Grafana-Standard, nicht abschaltbar. Zeitraum fest via `&from=now-6h&to=now` (kein Timepicker im Kiosk).
