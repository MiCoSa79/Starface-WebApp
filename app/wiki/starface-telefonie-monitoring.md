...
| Version | Datum | Inhalt |
|---|---|---|
| v0.0.141 | 2026-08-26 | **Diagnose „Nicht installiert trotz Installation“:** Befund Testanlage — beide Module rot „Nicht installiert“, obwohl v5 eingerichtet (GetModuleStatus antwortet, IST leer/Namens-Mismatch; kein Fehler-Banner). Neuer Admin-Endpoint `GET /api/monitoring/module-status-raw?installation=<name>` (401/403/404-Checks) + Admin-Diagnose-Block auf der Monitoring-Seite: rohes `moduleJson`/Fault ohne Interpretation. Fix folgt nach Befund (raw=[] → Modul-Seite, Namens-Diff → Matching, fault → Klassifikation). |
