...
1. **SOLL-Module** liest die WebApp automatisch aus `app/modules/*.sfm` (jede gelieferte `.sfm` = ein erwartetes Modul; SOLL-Version = `module-descriptor.xml` → `version`). Aktuell: `CallBlocker` v28 + `TelefonieMonitoring` **v7**.
