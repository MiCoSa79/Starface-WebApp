...
### F4: Update-Server-URL ist eine ADMIN-EINSTELLUNG (seit v0.0.121 etabliertes Muster)
...
- **✅ P5 (30.08., UMGSETZT — dm-v10 + WebApp v1.0.89):** **Anlagen-Updates ausgeliefert** — Modul-RPCs `[Instanz].GetAnlagenUpdates` (read-only) + `[Instanz].ExecuteAnlagenUpdate` (Validierung gegen frische Update-Liste, Update-Pipeline in eigenem Thread mit 2 s Delay, Antwort VOR `logoutAll(SERVER_UPDATE)`), WebApp-Administration **„Anlagen-Updates“** (`/admin/anlagen-updates`, neuer Nav-Punkt): verfügbare Updates abfragen, **sofort installieren** oder **planen** (datetime-local in Europe/Berlin, Speicherung/Vergleich in UTC — Sommer-/Winterzeit-Tests), Scheduler `anlagen_update_scheduler.py` (30-s-Tick, Status `planned/executed/error/missed/cancelled`, kein stilles Nachholen bei > 5 min Überfälligkeit), DB-Tabelle `anlagen_update_plans`. Details: [[anlagen-update-deployment]]
