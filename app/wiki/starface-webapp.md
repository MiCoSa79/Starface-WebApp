...
**Stand 29.08. (F70, v1.0.63):** **„Blocklist bearbeiten“ als eigene Spalte in der Modul-Status-Tabelle.** Auf der Anlagen-Detail-Seite `/installation/{id}` wanderte der Einstellungs-Button aus der separaten Karte „Modul-Einstellungen“ (entfernt) **in die Modul-Status-Tabelle als fünfte Spalte „Einstellungen“** — und erscheint **nur beim jeweiligen Modul, wenn es installiert UND eine Instanz aktiv ist** (serverseitig in `installation_detail_page` gesetzt: CallBlocker → `settings = {label: Blocklist bearbeiten, url: /installation/{id}/blocklist}`; Template: `{% if it.settings %}<a class="btn-secondary">📞 …</a>{% else %}—{% endif %}`). CallBlocker ohne aktive Instanz (Badge „Keine aktive Instanz“) bekommt KEINEN Button — nur „—“. Wenn keine Monitoring-Daten vorliegen (Fehler/Anlage down) bleibt die Tabelle wie bisher (Hinweis-Badge). Tests: Suite 30 Dateien grün — `installation_detail_test` +2 Checks (Button **in** der Zeile, genau 1×; CallBlocker-Instanz deaktiviert → KEIN Button) und „Modul-Einstellungen“-Karten-Check auf „Spaltenkopf + keine Karte“ umgestellt. CI-Tag v1.0.63.
