...
| Modul-Updates | Variante 1 `/admin/updates` (`?inst_id=`), Variante 2 `/admin/updates/modul` (`?module=`), Aktionen `/push`, `/push-all`, `/ping`, Bulk `/push-module`, Anlagen-RPC `[Instanz].UpdateFromUrl` |
...
**Stand 29.08. (F73, v1.0.68):** **Modul-Updates in zwei Varianten** (Untermenü „Modul-Updates ▸“ im Admin-Dropdown — öffnet auf den Updates-Seiten automatisch). **Variante 1 „nach Anlage“** („Updates nach Anlage“): Such-Combobox — leerer Zustand nur Combobox + Hinweis „Bitte oben eine Anlage auswählen“; mit `?inst_id=` wird NUR diese Anlage frisch gefetcht (`_collect_module_status`) und wie bisher angezeigt (Deployment-Modul-Hinweis, Sammel-Buttons installieren/aktualisieren, Tabelle Modul/SOLL/IST/Aktion mit „Update anstoßen“ + „Download-Test“); alle Aktionen (`push`/`push-all`/`ping`) redirecten mit `inst_id` zurück — die Auswahl bleibt nach der Aktion erhalten. **Variante 2 „nach Modul“** („Updates nach Modul“): Such-Combobox über das Modul-Sortiment; leerer Zustand Hinweis „Bitte oben ein Modul auswählen“; mit `?module=` listet die Seite ALLE Anlagen, die das Modul installiert haben und nicht aktuell sind (IST frisch je Anlage; ohne Monitoring-Instanz übersprungen; nicht erreichbare Anlagen als Meldung; „Aktuellste Version“ = SOLL): Tabelle **Auswahl-Checkbox | Anlage | Version (IST) | Aktuellste Version | Aktion** — Zeilen-Button „Update anstoßen“ nur mit Deployment-Modul (sonst Hinweis „— kein Deployment-Modul“), **„Alle auswählen“/„Auswahl aufheben“** (JS) und Bulk-Button **„Update auf allen ausgewählten Anlagen anstoßen“** → `POST /admin/updates/push-module` (`module_name`/`filename`/`version` + `installation_ids[]`; ohne Auswahl Hinweis; Meldung mit Anzahl + Fehlern; Redirect behält `?module=`). Design: `.msg`/`.empty`/`.muted` + `td form`-Regeln jetzt GLOBAL in admin.css (waren seitenlokal), `.cb`-Combobox-Muster aus rechte.html übernommen, keine neuen UI-Bauteile. Tests: **36 neue Checks** in `admin_updates_v2_test.py` (Submenü, beide Leerzustände, Filter „installiert + nicht aktuell“, Checkboxen/Bulk, push-module gemockt, Redirect-Kontexte), bestehende Updates-Tests auf `?inst_id=`/`?module=`-Aufrufe umgestellt; Gesamtsuite grün. CDP-Belege: `f73_shots/v1_leer|v1_auswahl|v2_leer|v2_tabelle|nav_submenu` (Leerzustände, Auswahlansicht, Bulk-Zustand mit angehakten Checkboxen, Untermenü).
