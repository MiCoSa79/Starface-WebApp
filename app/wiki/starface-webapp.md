...
**Stand 29.08. (F74, v1.0.69):** **Comboboxen öffneten sich nicht — Fix.** Beide Modul-Updates-Seiten (V1/V2) banden `admin.js` **nicht** ein — nur `admin.css` ist global in base.html; `initComboboxes` (datalist-Such-Combobox) fehlte daher komplett, die `.cb-trigger`-Buttons waren tot (von Axel live gemeldet). Fix: `<script src="/static/admin.js?v={{ version }}" defer>` in `admin_updates.html` + `admin_updates_modul.html` (Cache-Busting-Konvention v1.0.67). **Nachweis per CDP mit echtem Klick:** V1-Dropdown öffnet (2 Item-Zeilen), Klick `?inst_id=1`, Redirect greift; V2-Dropdown öffnet (3 Module), Klick `?module=CallBlocker`, Tabelle mit Zeilen + Bulk-Buttons. **2 neue Regressions-Checks im v2-Test** („admin.js eingebunden") — 38 Checks grün, Suite grün. CI-Tag v1.0.69 nach Push.
