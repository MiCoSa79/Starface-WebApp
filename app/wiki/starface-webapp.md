...
**Stand 29.08. (F75, v1.0.70):** **Button-Ausrichtung + Spaltenbreite in beiden Modul-Updates-Tabellen** (Axel: „Button-Ausrichtung und Breite anpassen. Sieht nicht schön aus“ — Screenshot der V1-Tabelle). Die Aktions-Spalte frisst die Restbreite der 100-%-Tabelle, Buttons standen links mit toter Zone. Fix: Aktions-Zellen `text-align:right; white-space:nowrap` (Referenz: Einstellungen-Spalte der Modul-Tabelle in installation_detail, v1.0.70-Muster) + Formular `inline-flex gap:6px` in V1 (zwei Buttons) + **letzter Tabellen-Kopf `width:1%; text-align:right`** → „Aktion“-Spalte schrumpft auf Inhalt, freie Breite geht an die Modul-Spalte (keine Namens-Umbrüche). Gilt für V1 (`admin_updates.html`) und V2 (`admin_updates_modul.html`). Nachweis: CDP Desktop 1280 (Buttons kompakt direkt nach IST, Spalte ~20–25 % statt Rest, keine Umbrüche) + Mobil 390 (Tabelle o. horizontales Scrollen-Clipping der Texte); Suite grün (updates/v2/layout). Davor **F74 (v1.0.69):** Combobox-Fix — `admin.js` (initComboboxes) in beide Updates-Templates eingebunden + 2 Regressions-Checks.
