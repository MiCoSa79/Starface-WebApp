...
CallBlocker-Pflege, Telefonie-Monitoring (InfluxDB, nativ visualisiert), Modul-Verwaltung und
...
| Modul-Updates | Variante 1 `/admin/updates` (`?inst_id=`), Variante 2 `/admin/updates/modul` (`?module=`), Variante 3 „Standard-Module“ `/admin/updates/standard` (+ `/standard/push`; alte URL `/fehlende` → 303), Aktionen `/push`, `/push-all`, `/ping`, Bulk `/push-module`, Anlagen-Ausnahme `POST /installation/{id}/module/standard` (JSON `{module, active}`), Anlagen-RPC `[Instanz].UpdateFromUrl` |
...
**Stand 29.08. (F77, v1.0.74):** **„Standard entfernen“ je Modul-Zeile auf der Standard-Module-Seite.** Jede Zeile von `/admin/updates/standard` bekommt in der Aktions-Spalte einen `btn-mini`-Button **„Standard entfernen“** (`data-inst`/`data-module`, Tooltip, Inline-JS `removeStandard(this)`): setzt per `POST /installation/{id}/module/standard` `{module, active:true}` die **Anlagen-Ausnahme** (bestehende v1.0.73-API) — das Modul verschwindet sofort aus der Liste dieser Anlage, der **globale Standard bleibt unverändert**; aufhebbar über die Detailseite (dortiger Toggle). Auch Zeilen ohne Deployment-Konfig haben den Button. Verifikation: 5 neue Checks 5e–5i (`standard_module_test.py` → 39 Checks; Button-Anzahl 5→4 nach Ausnahme, POST ok, data-Attribute), wiki_e2e 51 + monitoring_rechte_e2e 18 grün. Davor **v1.0.73 (F76/3, Axel-Rename):** „Fehlende Module“ → **„Standard-Module“** — Menüpunkt + Unterseite (Route `/admin/updates/standard`, Template `admin_updates_standard.html`, active `updates-standard`, Alt-URL `/admin/updates/fehlende` → 303) und **Anlagen-Ausnahmen**: Tabelle `module_default_overrides(installation_id, module_name)` (PK beide), Helper `_standard_module_names(conn, inst_id)` = `is_standard` minus Overrides (EINE Quelle für Standard-Seite, `action=all`-Server-Fallback und Detailspalte), Detailseiten-Spalte „Standard“ (grüne Pill „Standard“ / orange „Ausnahme“ + `btn-mini`-Toggle) + `POST /installation/{id}/module/standard` (Admin, JSON; 400 kein Standard-Modul, 404 unbekannte Anlage; `tojson` NIE in `onclick` — `data-module`). Davor **v1.0.72 (F76/2): Fehlende-Module-Seite** (dritte Untermenü-Position; Gruppen je Anlage, `.std-cb`/`bulk-standard`, `POST /admin/updates/fehlende/push` mit `action=all|selected`, Badge „⚠ Deployment-Modul fehlt“, Leerzustände).

**Stand 29.08. (F76/1, v1.0.71):** **Standard-Flags auf der Modul-Seite (Schritt 1/3 „Fehlende Module“).** Neue Spalte `modules.is_standard` (Migration in init_db, `INTEGER DEFAULT 0`) + `POST /admin/modules/standard` (Admin-only, `{name, active}` → JSON `{ok}`; unbekanntes Modul → `ok:false`, ohne name → 400). Die Modul-Seite (`/admin/modules`) hat in **beiden** Tabellen (eigene + Drittanbieter) eine Spalte **„Standard“** mit zentrierter Checkbox (Tooltip „Standard: Modul muss auf jeder Anlage installiert sein“); Toggle per Inline-JS-fetch mit Rollback bei Fehler. Bedeutung: Standard = Modul soll auf JEDER Anlage installiert sein (Grundlage für F76/2 „Fehlende Module“-Seite). Tests: `tmp_tests/module_standard_test.py` 12 Checks (Migration, beide Tabellen render, Set/Löschen/DB-Beleg, checked nach Setzen, unbekannt/ohne name, Nicht-Admin-Redirect) — Suite komplett grün (33 Dateien inkl. E2E + wiki_e2e); CDP-Klick-Nachweis: CallBlocker an → checked, nach Reload persistiert. Davor **F75 (v1.0.70):** Button-Ausrichtung + Spaltenbreite in beiden Modul-Updates-Tabellen (Aktions-Zellen rechtsbündig+nowrap, letzter Kopf `width:1%` — Referenz installation_detail; V1-Formular `inline-flex gap:6px`).

**Stand 29.08. (F64, v1.0.56):** **Admin-Monitoring ist die Startseite; Anlagen-Übersicht läuft unter `/anlagen`; Benutzer-Menü wandert in die Topbar.** `/` zeigt Admins das Admin-Monitoring (`admin_monitoring.html`: Kennzahlen, Fehlerliste, Vollbild, Auto-Refresh) und leitet normale Benutzer auf `/anlagen` weiter; nicht eingeloggt → Login. Neue Route `GET /anlagen` = Anlagen-Übersicht für alle eingeloggten (Admins: Anlegen-Formular + Tabelle mit Suchfeldern + Aktionen; User: nur eigene Anlagen mit `.detail-dl`/Blocklist/Test, kein Formular; Rendering `anlagen.html` unverändert, nur URL neu). **Navigation:** das Logo steht nur noch im Seitenkopf (Topbar); die Navigationsleiste enthält **Startseite** (→ `/`, erster Eintrag, nachgereicht v1.0.57: Admins = Gesamt-Monitoring, User = ihre Startseite via Redirect), die flachen Links **Anlagen** (→ `/anlagen`), **Monitoring** und das **Administration ▾**-Dropdown — **ohne Logo-Link und ohne Benutzer-Dropdown**. Stattdessen sitzen **Benutzereinstellungen + Abmelden** als Dropdown **oben rechts am Benutzernamen** (`details.user-drop` in der Topbar: „Mein Konto“ → `/konto`, „Abmelden“ → `/logout`; funktioniert auch mobil, Hamburger bleibt). Redirects vereinheitlicht: POST Anlagen-CRUD (create/update/delete/edit-not-found) + Zurück-Links (Blocklist, Bearbeiten, Admin-Monitoring, Modul-Updates) → `/anlagen`; Passwort-Rückmeldungen (`?pw_ok`/`?pw_err`) → `/anlagen?…`; Schutz-Guards zeigen weiter auf `/`; Login-JS navigiert direkt auf `/` statt über den Alt-Umweg `/dashboard`. Tests: Suite 29 Dateien grün; angepasst: `admin_aufteilung_e2e` (Startseite = Admin-Monitoring, `/anlagen`-Verträge, Redirects), `monitoring_rechte_e2e` (#7–#10 + #12 auf `/anlagen` umgestellt + Startseiten-Wächter), `konto_e2e` (Topbar-Dropdown), `wiki_e2e` (User-Redirect), `admin_filter_e2e`/`badge_e2e`/`e2e_update` (GET `/anlagen`); CDP `menu_umbau_cdp.mjs` 26/26 (auch Startseite-Link: erster Nav-Eintrag, Klick `/anlagen`→`/` Gesamt-Monitoring, Normaluser-, Mobile-Check), `mobile_url_layout_cdp.mjs` 7/7.

**Stand 29.08. (F71, v1.0.64):** **Stammdaten + app-weite Design-Konsistenz.** Die **Stammdaten-Karte** der Anlagen-Detail-Seite `/installation/{id}` zeigt jetzt: Name, URL, API-Version, **Anlagen-Version** (frisch via GetStats `systemVersion`, Route `installation_detail_page`), **Deployment-Modul** (Badge: „installiert und aktiv“ / „nicht installiert“ / „keine aktive Instanz“ / „Konfiguration nötig“ / „nicht erreichbar“ — Status aus demselben `_collect_module_status`-Lauf wie die Modul-Tabelle, serverseitig `dep_state`) und **Update-Token** (gesetzt/nicht gesetzt). Die früheren Zeilen **„CallBlocker-Instanz“/„TelefonieMonitoring-Instanz“/„Deployment-Modul-Instanz“ sind entfernt** — die übrigen Module stehen ohnehin in der Modul-Status-Tabelle darunter. **Design-Konsistenz (Wurzel der „zerschossenen“ Optik):** `admin.css` wurde nur in 4 Templates geladen (anlagen/benutzer/grundeinstellungen/rechte) — auf Detail-, Edit-, Monitoring-, Blocklist-, Passkey-Seiten gab es **keine** `.btn-secondary`-Definition → alle Button-*Links* waren nackte blaue Browser-Links. Fix: `admin.css` **global in `base.html`** eingebunden (+ Duplikat-Links aus den 4 Templates entfernt), `a.btn-secondary/a.btn-danger` bekommen volle Button-Optik (Padding 8px 16px, inline-block, keine Unterstreichung — probegleich mit `<button>`), neue Klasse **`.btn-back`** (btn-secondary + margin-bottom 16px) für alle Zurück-Links (installation_detail, edit_installation, admin_monitoring, installation_monitoring, monitoring, blocklist, passkeys, otp_setup; lokale `.back`-Regeln entfernt). Die Einstellungen-Spalte der Modul-Tabelle ist jetzt **rechtsbündig + `white-space:nowrap`** wie die Aktions-Spalte der Anlagen-Tabelle. Nachweis: CDP-computedStyle (`a.btn-back`: padding 8px 16px, bg #333, radius 6px, inline-block; Blocklist-Button identisch) + Suite 30 Dateien grün (installation_detail_test auf „Stammdaten: Anlagen-Version + Deployment + Token“ umgestellt, CallBlocker/TelefonieMonitoring-Instanz-Zeilen negativ geprüft). CI-Tag v1.0.64.

**Stand 29.08. (F69, v1.0.62):** **Auge-Link zur Monitoring-Detailseite wandert in die Tabellen-Zeile.** Der (mit F68 auf die Edit-Seite gesetzte) Auge-Button zur Monitoring-Detailseite sitzt jetzt **in der Anlagen-Tabelle in jeder Zeile VOR dem „Zur Anlage“-Button** (Admin- UND User-Zeile: `.detail-dl`-Icon-Link → `/monitoring/installations/{id}`, gleiche Auge-SVG; die Admin-Zeile hatte vorher keinen detail-dl). Die Bearbeiten-Seite `/admin/installations/{id}/edit` behält nur den „← Zurück zur Anlagen-Übersicht“-Button (oben) — ohne Monitoring-Link. Tests: Suite 30 Dateien grün — `installation_detail_test` +1 Check (Tabelle: Auge VOR „Zur Anlage“ via `body.index`-Vergleich) und Edit-Seiten-Check auf „KEIN Monitoring-Link“ umgestellt; `monitoring_rechte_e2e` + Positions-Assert (Bob: Auge vor Zur-Anlage). CI-Tag v1.0.62.

**Stand 29.08. (F68, v1.0.61):** **Einheitliche Aktions-Buttons + Edit-Seiten-Kopf.** Anlagen-Tabelle: ⚡-Test-Button hat jetzt exakt die Optik von „Zur Anlage“/„Edit“ (`btn-secondary`, bg #333 — vorher transparenter Hintergrund mit Rahmen), alle vier Aktions-Buttons der Zeile haben abgerundete Ecken (6 px wie „Löschen“); die verstreuten Inline-Styles sind raus (zentrale `.actions-inline a/button`-Regel in `anlagen.html`, `btn-secondary` global `border-radius:6px` in admin.css). Bearbeiten-Seite `/admin/installations/{id}/edit`: oben **„← Zurück zur Anlagen-Übersicht“**-Button (→ `/anlagen`) und **Auge-Button zur Monitoring-Detailseite** `/monitoring/installations/{id}` (gleiche Auge-SVG wie die Tabelle). Tests: Suite 30 Dateien grün (`installation_detail_test` +3 Checks: Edit-Seiten-Kopf, Auge-Link, keine Inline-Styles mehr in der Zeile); CDP-Nachweis: alle 4 Buttons bg #333/6 px, Edit-Kopf mit beiden Links. CI-Tag v1.0.61.

**Stand 29.08. (F67, v1.0.60):** **Anlagen-Detail-Seite + Deployment-Modul-Test + Gesamt-Monitoring.** Neue Route `GET /installation/{inst_id}` (eingeloggte mit `can_read`) = **Detail-Seite einer Anlage** (`installation_detail.html`): **Stammdaten-Karte** (Name, URL, API-Version, CallBlocker-/TelefonieMonitoring-/Deployment-Instanz, Update-Token gesetzt/nicht gesetzt), **Modul-Status-Karte** (die seit F65 geparkte Karte — jetzt WIEDER EINGEBAUT als Einzel-Anlagen-Ansicht, serverseitig aus `monitoring._collect_module_status`, kein Auto-Refresh: Status-Badges Aktuell/Update/Fehlt/Keine aktive Instanz, Version IST → SOLL, Instanzen) und **Modul-Einstellungen-Karte** (zukunftsfähig; aktuell: **„Blocklist bearbeiten“** für das CallBlocker-Modul — Link nur, wenn das Modul installiert ist). Anlagen-Tabelle `/anlagen`: **Blocklist-Button entfernt** (kommt über die neue Detail-Seite, wenn CallBlocker installiert), neuer Button **„Zur Anlage“** (→ `/installation/{id}`, Admin + normale Benutzer); das Auge-Icon (Detail-Monitoring `/monitoring/installations/{id}`) bleibt. **Test-Button prüft neu das Deployment-Modul**: `_deployment_modul_status()` (main.py) macht Live-Checks gegen die Anlage — `GetModuleStatus` über die TelefonieMonitoring-Instanz, sucht das Deployment-Modul anhand der SOLL-Liste (`_module_expectations`, „deployment“ im Namen), bewertet `missing`-Status als „nicht installiert“ und prüft aktive Instanzen; Antworten: **✅ Deployment-Modul installiert und erreichbar (Instanz: …)** / ⚠️ **nicht installiert** / ⚠️ **installiert, aber keine Instanz aktiv** / ⚠️ **config** (Monitoring-Instanz fehlt) / ❌ **Anlage nicht erreichbar**. Betrifft beide Endpunkte: `GET /installation/{id}/test` (User) und `GET /admin/installations/{id}/test-conn` (Admin-Tabellen-Button); `testConn()` in admin.js zeigt grün/orange/rot je `state`. Startseite: Überschrift „Admin-Monitoring“ → **„Gesamt-Monitoring“** (admin_monitoring.html h2 + title). Tests: Suite 30 Dateien grün (je 29 + neu `installation_detail_test.py` = 17 Checks: Deployment-Status-Fälle ok/not-installed/no-active-instance/config/unreachable, Detail-Route 200 + Marker, Blocklist-Link nur bei installiertem CallBlocker, „Zur Anlage“ statt Blocklist in der Tabelle, Redirects; angepasst: `admin_aufteilung_e2e`, `monitoring_rechte_e2e`, `admin_monitoring_test`, `e2e_update` (Anlagen-ID nun über „Zur Anlage“-Link), CDP `menu_umbau_cdp.mjs` („Gesamt-Monitoring“)); CDP-Nachweise `f67_*.png` unter 01_uploads.

**Stand 29.08. (F66, v1.0.59):** **Admin-Tabellen standardmäßig aufgeklappt.** Die Einklapp-Mechanik (seit v0.0.97, damals als alles auf einer Seite war) zeigt die 3 Admin-Tabellen (Anlagen/`inst`, Benutzer/`users`, Rechte/`access`) wieder **OFFEN** an: in `static/admin.js`-`initCollapse` ist der Standard jetzt `collapsed=false` (lokalStorage nur noch `'0'` = explizit ZUgeklappt). **Zuklappen per Chevron-Button + Persistenz bleiben weiter nutzbar**; statisches `aria-expanded="true"` in den Templates passte bereits. Tests: Suite 29 Dateien grün (admin_filter_e2e unverändert), CDP `admin_filter_cdp.mjs` **52/52** (Abschnitt 6 auf „Initial offen, aria-expanded=true, Klick = zuklappen, Reload-Persistenz, kein Wert → offen“ umgedreht) + `menu_umbau_cdp.mjs` 29/29; Preview-Fixture `/opt/data/admin-preview/admin_test.html` (Inline-JS-Kopie) synchronisiert.

**Stand 29.08. (F65, v1.0.58):** **Menü verschlankt + Monitoring-Seite umgebaut.** Der flache Nav-Link **Anlagen** ist entfernt (Anlagen erreichbar über das **Administration ▾**-Dropdown; für Normaluser führt der Startseite-Link weiter auf ihre Anlagen) — Navigation jetzt: **Startseite, Monitoring, Administration ▾**. Monitoring-Seite `/monitoring`: Überschrift **„Monitoring-Übersicht“** (statt „Telefonie-Monitoring“); die **Modul-Status-Karte ist GEPARKT** (kommt später woanders hin, z. B. eigene Seite/Admin-Bereich — der Aufbau ist vollständig dokumentiert in der Skill-Referenz `modul-status-karte-geparkt-v158.md`; Datenfluss unverändert: der Sammler pollt `GetModuleStatus` weiter in den Cache, `/api/monitoring/status` und die Detail-Seiten liefern `modules` weiter). Grundeinstellungen: Button **„Admin-Monitoring öffnen“** entfernt (Admin-Monitoring ist seit F64 die Startseite). Tests: Suite 29 Dateien grün; CDP `menu_umbau_cdp.mjs` **29/29** (3 neue Checks: kein flacher Anlagen-Link in Nav/Hamburger — Admin, Normaluser, Mobile) + `mobile_url_layout_cdp.mjs` 7/7; Nachweis-Screenshots `f65_*.png` unter 01_uploads.
...
error_box 9) + CDP `mobile_url_layout_cdp.mjs` 7/7 + `menu_umbau_cdp.mjs` 19/19.

**28.08. (v1.0.54, Admin-Aufteilung):** **Dashboard raus — Startseite `/` = Anlagenübersicht, Admin-Bereich in 4 Seiten aufgeteilt.** Die Admin-Seite (`/admin`, bisher 4 Sektionen in einer Seite) und das Dashboard (`/dashboard`) sind aufgelöst: **`/`** ist jetzt die Anlagen-Übersicht für **alle** eingeloggten — Admins sehen Formular (Anlage hinzufügen) + vollständige Tabelle mit Suchfeldern und Aktionen (Blocklist, Test, Edit, Löschen); normale Benutzer sehen wie im alten Dashboard nur **ihre** Anlagen (mit Grafana-, Blocklist- und Test-Aktion, ohne Anlegen-Formular). Neue Admin-Seiten: **`/benutzer`** (Benutzer anlegen + Benutzerkonten + Suche, 2FA/Passkeys/Passwort-Reset), **`/rechte`** (Rechte vergeben + Liste der gesetzten Berechtigungen), **`/grundeinstellungen`** (Basis-Domains Grafana/Update-Server + Admin-Monitoring-Einstieg). Die Alt-Routen `/dashboard` und `/admin` leiten auf `/` um (36 Schutz-/POST-Redirects sowie OAuth- und Passwort-Flows zeigen auf die neue Startseite bzw. die passende Bereichs-Seite: Benutzer-Änderung → `/benutzer`, Recht → `/rechte`, Einstellung → `/grundeinstellungen?set_ok=1`). Navigation: flacher **„Anlagen“-Logo-Link** (→ `/`) + `[Monitoring] [Administration ▾]` mit **Trennlinie** (Anlagen, Benutzer, Rechteverwaltung, Grundeinstellungen | Module, Modul-Updates, API-Doku, Wiki) + `[👤 ▾]`. CSS/JS der Admin-Ansichten in `static/admin.css` + `static/admin.js` (eine Quelle statt 1× dupliziert). Tests: neues `tmp_tests/admin_aufteilung_e2e.py` (Startseite, 4 Seiten, Redirect-Verträge, User-Sicht), `admin_filter_e2e.py` auf 4 Seiten verteilt, `build_admin_preview.py` rendert alle 4 Bereiche; komplette Suite (28 Testdateien) + CDP `menu_umbau_cdp.mjs` (19 Checks) grün.
**Stand 28.08. (Menü-Umbau, v1.0.53):** **Aufgeräumte Menüführung + „Mein Konto“ als Self-Service.** Die flache Navigationsleiste (8+ Einzel-Links, Admin- und Benutzer-Links gemischt) ist ersetzt durch **4 Einstiege** (base.html): `[Dashboard]`, `[Monitoring]`, `[Administration ▾]` (nur Admin: Admin, Module, Modul-Updates, API-Doku, Wiki), `[👤 Benutzername ▾]` (alle: „Mein Konto“, „Abmelden“). Umsetzung als native `<details>/<summary>`-Dropdowns (Klick, kein Hover — funktioniert auf Touch): aktiv gefärbt, wenn eine Seite der Gruppe offen ist. Mobile (≤640 px) werden die Dropdowns im Hamburger zu aufklappbaren Untergruppen (statisch positioniert, kein Overlay). **Neue Seite `GET /konto` („Mein Konto“, für alle eingeloggten):** Sektion Profil (Benutzername, Rolle) + Sektion Sicherheit — **Passwort ändern** (Re-Auth: aktuelles Passwort nötig), **2FA (TOTP)** einrichten (QR + Secret, einmalige Backup-Codes mit 5-min-TTL nach Bestätigung) / deaktivieren (Re-Auth), **Passkeys** verwalten (Liste/Registrieren/Entfernen über bestehende User-eigene APIs). Der Admin-Weg (Selbstbedienung für Fremduser, Reset/Setup/Disable als Support-Fall) bleibt unverändert. `GET /password` ohne `?uid=` ist jetzt ein Redirect auf `/konto#sicherheit` (alte Links, Lesezeichen und POSTs zu `password.html` bleiben damit intakt); der Admin-Reset-Modus `GET /password?uid=<fremd>` rendert die schmale 520-px-Seite weiter. Tests: `tmp_tests/konto_e2e.py` (40+ Checks: Nav-Rechte, Redirects, Passwort-/2FA-/Passkey-Flows inkl. Re-Auth und Einmal-Anzeige der Backup-Codes), `tmp_tests/menu_umbau_cdp.mjs` (17 Checks, Desktop + Mobile 390×844: Dropdowns auf-/zuklappen, Hamburger-Gruppen, kein flacher /password-Link), bestehende Suite inkl. `admin_layout_width_test.py` an neue Verträge angepasst (520 px nur noch Admin-Reset-Modus, `/konto` 1400 px).
...
| Telefonie-Monitoring (Modul + InfluxDB) | [starface-telefonie-monitoring](starface-telefonie-monitoring.md) |
...
|---|---|---|
| v1.0.74 | (29.08.) | feat(F77): **„Standard entfernen“-Button je Zeile der Standard-Module-Seite** — setzt die Anlagen-Ausnahme via `POST /installation/{id}/module/standard` `{active:true}` (globale Standard-Flags unverändert; Modul verschwindet aus der Liste dieser Anlage, aufhebbar auf der Detailseite); 5 neue Checks 5e–5i (standard_module_test → 39 grün), wiki_e2e 51 + monitoring_rechte_e2e 18 grün; Repo-Wiki-Nachtrag F76/2+F76/3 | – |
| v1.0.73 | (29.08.) | feat(F76/3, Axel-Rename): **„Fehlende Module“ → „Standard-Module“** (Menüpunkt + Seite `/admin/updates/standard` + `/standard/push`, Template-Rename `admin_updates_standard.html`, Alt-URL → 303) + **Anlagen-Ausnahmen** `module_default_overrides` + Helper `_standard_module_names(conn, inst_id)` (eine Quelle für Seite/Fallback/Detail), Detail-Spalte „Standard“ (Pill/Toggle) + `POST /installation/{id}/module/standard`; 34 Checks grün, CDP f76c_shots, Commit `99558e2` | – |
| v1.0.72 | (29.08.) | feat(F76/2): **Fehlende-Module-Seite** (dritter Untermenü-Punkt, ab v1.0.73 „Standard-Module“) — Gruppen je Anlage, Module fehlend/veraltet (Standard ∩ expectations), Anlagen-Checkbox, Bulk `POST /admin/updates/fehlende/push` (action=all JS + Server-Fallback, Dedup, kein-Deployment-Fehler ohne RPC), Badge „⚠ Deployment-Modul fehlt“; 22 Checks grün, CDP f76b_shots, Commit `a0719bd` | – |
| v1.0.71 | (29.08.) | feat(F76/1): **Standard-Flags auf der Modul-Seite** — `modules.is_standard` (Migration), `POST /admin/modules/standard` (JSON ok, Rollback unbekannt/kein name), Checkbox-Spalte „Standard“ in beiden Tabellen (eigene + Drittanbieter), Inline-fetch-Toggle; 12 neue Tests (module_standard_test), Suite 33 grün, CDP-Klick-Nachweis mit Reload-Persistenz | – |
| v1.0.70 | (29.08.) | fix(F75): **Button-Ausrichtung + Tabellenbreite** in beiden Modul-Updates-Ansichten — Aktions-Zelle rechtsbündig + nowrap (Referenz installation_detail), letzter Kopf `width:1%` → Aktion-Spalte kompakt, freie Breite an die Modul-Spalte (keine Umbrüche); CDP Desktop+Mobil, Suite grün | – |
| v1.0.69 | (29.08.) | fix(F74): **Comboboxen öffneten sich nicht** — `admin.js` (initComboboxes) in beide Modul-Updates-Templates eingebunden (`?v=`-Cache-Busting); 2 Regressions-Checks im v2-Test; CDP-Nachweis mit echtem Klick (Dropdown öffnen + Auswahl + Redirect) | – |
| v1.0.68 | (29.08.) | feat(F73): **Modul-Updates in zwei Varianten** — Untermenü „Modul-Updates ▸“; V1 `/admin/updates?inst_id=` (Such-Combobox, Leerzustand, Auswahl bleibt nach Aktionen), V2 `/admin/updates/modul?module=` (alle Anlagen mit nicht aktuellem Modul, Checkboxen + Alle/Nichts + Bulk-`/push-module`); `.msg/.empty/.muted/td form` global in admin.css; 36 neue v2-Tests (admin_updates_v2_test.py), Updates-Tests auf Param-Aufrufe umgestellt, Gesamtsuite grün; CDP-Belege f73_shots | `c8e4314` |
| v1.0.67 | (29.08.) | fix(F72-Folge): **Cache-Busting für Statics** — alle `/static/*.css`/`*.js` mit `?v={{ version }}` (neue URL je Release; base.html + anlagen/benutzer/rechte/installation_detail für admin.js) + **`Cache-Control: no-cache` auf `/static`** (Middleware in main.py; ETag/Last-Modified bleiben → 304-Revalidierung) — behebt „alte Optik trotz geleertem Browser-Cache“ (Caching-Ebene hielt die alte admin.css; erst Strg+F5 brachte den neuen Stand). **+ Startseiten-Header bündig**: `.btn-back` aus der Flex-Zeile genommen (sein margin-bottom:16px versetzte den Zurück-Button im `align-items:center`-Container gegenüber „Vollbild“; Container hat selbst margin-bottom). admin_filter_e2e wartet auf den `?v=`-Link. Nachweis: CDP deltaTop/deltaHöhe = 0 (161.0/34.2 beidseitig), curl-Cache-Header + 304. `ea02eb8` |
| v1.0.66 | (29.08.) | fix(F71-Folge): **EIN Button-Design app-weit — farbiger Hintergrund + abgerundete Ecken** (Axel: „Zurück-Button der Startseite = Referenz für alle Buttons; Farben ok wie sie sind“). Neues zentrales Basis-Rule-Set in admin.css (inline-flex+gap, padding 8×14, radius 6, font 13/600) für .btn-primary/-secondary/-danger + Hover (secondary-Hover = Startseiten-Dunkelblau #0f3460/weiß) + .btn-mini (kompakte Variante). Löschen-Button bleibt dunkelrot, primary bleibt rot. ALLE lokalen Button-Überschreibungen entfernt: edit_installation (Outline-Button `background:none` → war die „Katastrophe“), modules (Rotbraun-Ton), blocklist/passkeys-Duplikate, admin-/installation_monitoring-Duplikate inkl. Media-Query; Inline-Styles raus (benutzer 2FA/Passkeys/Passwort + OK→btn-mini; edit/otp Zurück). Nachweis: CDP-Mobile (390px) alle Seiten — btn-back überall bg #333, radius 6, gleiche Maße; Hover-Beweis #0f3460/weiß; Suite 30 grün. |
...
| v1.0.61 | (29.08.) | feat(F68): **Einheitliche Aktions-Buttons in der Anlagen-Tabelle** (Test = Zur Anlage = Edit: gleiche `btn-secondary`-Optik, 6-px-Radius überall, Inline-Styles raus — zentrale `.actions-inline`-Regel + `btn-secondary`-Radius global) + **Edit-Seiten-Kopf** mit „← Zurück zur Anlagen-Übersicht“; Suite 30 Dateien grün (installation_detail_test +3) |
| v1.0.60 | (29.08.) | feat(F67): **Anlagen-Detail-Seite `/installation/{id}`** (Stammdaten + Modul-Status-Karte aus dem Parkhaus + Modul-Einstellungen mit Blocklist bei installiertem CallBlocker) — `_deployment_modul_status()`-Check, Blocklist-Button aus der Anlagen-Tabelle, „Zur Anlage“-Button, Test prüft Deployment-Modul, Startseite „Gesamt-Monitoring“; Suite 30 Dateien grün (neu installation_detail_test), CDP 29/29 + 52/52 |
| v1.0.59 | (29.08.) | feat(F66): **Admin-Tabellen standardmäßig aufgeklappt** — `initCollapse`-Standard in admin.js von „eingeklappt“ auf „aufgeklappt“ gedreht (lokalStorage nur `'0'` = explizit zugeklappt; Chevron-Zuklappen + Persistenz bleiben). Historie: Default war seit v0.0.97 eingeklappt (Ein-Seiten-Ära). Tests: Suite 29 grün, CDP `admin_filter_cdp.mjs` **52/52** (Collapse-Abschnitt umgedreht) + `menu_umbau_cdp.mjs` 29/29; Preview `admin_test.html` synchronisiert. |
| v1.0.58 | (29.08.) | feat(F65): **Menü verschlankt + Monitoring-Seite umgebaut** — flacher Nav-Link „Anlagen“ entfernt (nur noch im Administration-Dropdown; Nav: Startseite, Monitoring, Administration ▾); Monitoring-Seite: Überschrift **„Monitoring-Übersicht“**, **Modul-Status-Karte GEPARKT** (kommt später woanders hin; Sammler-Cache-Datenfluss unverändert). Grundeinstellungen: „Admin-Monitoring öffnen“-Button entfernt (ist die Startseite). Tests: Suite 29 grün, CDP `menu_umbau_cdp.mjs` 29/29 (3 neue Nav-Checks Admin/User/Mobile) + `mobile_url_layout` 7/7; Screenshots `f65_*.png` → 01_uploads. |
| v1.0.56 | (29.08.) | feat(F64): **Admin-Monitoring = Startseite, Anlagen unter `/anlagen`, Benutzer-Menü in der Topbar** — `/` rendert für Admins das Admin-Monitoring, Normaluser → Redirect `/anlagen`; neue Route `GET /anlagen` (Anlagen-Übersicht für alle, Anlegen nur Admin). Nav: Logo raus aus der Navigationsleiste (nur noch Topbar), flache Links Anlagen→`/anlagen`/Monitoring/Administration; Benutzer-Dropdown („Mein Konto“+„Abmelden“) in die Topbar rechts am Benutzernamen (`details.user-drop`). Redirects: Anlagen-CRUD + Zurück-Links → `/anlagen`, Passwort-Meldungen → `/anlagen?…`; Login-JS direkt auf `/`. Tests: Suite 29 grün, `admin_aufteilung_e2e`/`monitoring_rechte_e2e`/`konto_e2e`/`wiki_e2e`/`admin_filter_e2e`/`badge_e2e`/`e2e_update` angepasst, CDP `menu_umbau_cdp.mjs` 21/21 + `mobile_url_layout_cdp.mjs` 7/7. **Nachgereicht (v1.0.57):** Nav-Link „Startseite“ (→ `/`, erster Eintrag, title „Gesamt-Monitoring (Startseite)“; Admins = Gesamt-Monitoring, User = eigene Startseite via Redirect; Active-Marker `start`); CDP `menu_umbau_cdp.mjs` 26/26 (inkl. Klick-Test `/anlagen`→`/` + Normaluser- + Mobile-Check). |
...
| v1.0.51 | (27.08.) | fix(Axel, Screenshot 22:29): **„Module, die nicht aktuell sind“ — Inhalt repariert + Abstand symmetrisch** — Root Cause: der SOLL-Abgleich erkannte nur Module mit hinterlegtem Paket; Module, die auf der Anlage installiert sind, aber KEIN `.sfm`/DB-Eintrag haben, blieben unsichtbar (konnten nie als „nicht aktuell“ erscheinen). Fix: Sammler legt jetzt `raw_installed` (alle installierten Module laut Anlage) in den Cache; Admin-Auswertung ergänzt unbekannte installierte Module mit Soll „—“; bei `modules_error`/`list=None` zeigt die Sektion einen orangen Hinweis „Modul-Status nicht verfügbar“ statt fälschlich „alle aktuell“. Überschrift-Abstand: `#outdated-title { margin-top: -4px }` → 16 px Karten-Margin − 4 = 12 px = Abstand darunter (Kiosk wie Normal). Test: Fixture um Fremdmodul ohne Paket + Anlage mit modules_error erweitert, 23 neue/angepasste Checks, 19/19. |
| v1.0.50 | (27.08.) | feat(Axel): **Admin-Monitoring: neue Sektion „Module, die nicht aktuell sind“** — Daten AUSSCHLIESSLICH aus dem Sammler-Cache (kein Poll, wie Detail-Seite): `_admin_monitoring_summary` liefert je Anlage `outdated_modules` (nur installiert && nicht aktuell, sortiert) + `outdated_total`; Template: h2 im Detail-Stil + Karte, Gruppenzeile (`out-grp`, blau) je Anlage → Modulname / Ist-Version (rot) / Aktuellste Version; alle aktuell → grüner OK-Hinweis; JS-Refresh rendert mit `renderOutdated()` mit. Test: Fixture um Modul-Listen erweitert, 10 neue Checks (inkl. API outdated_total/outdated_modules), 19/19. |
| v1.0.49 | (27.08.) | fix/feat(Axel, Screenshot 22:13): (1) **Detail-Seite: Abstand über „Module“ = Abstand unter „Module“** — `#mod-sec-title { margin-top: 12px }` (war 2 px, vom letzten Wunsch „dichter“ — jetzt symmetrisch zur Karte unten; `h2`-margin-bottom 12 px bleibt). (2) **Admin-Kiosk: rote Überschrift „Gesamtübersicht aller Anlagen“** zentriert ganz oben — `.kiosk-title` (display:none, Kiosk: block; 24 px, `#e94560` wie Detail-Banner), nur Kiosk sichtbar, Normal unverändert. Test: 2 neue/angepasste Checks, 19/19. |
| v1.0.48 | (27.08.) | fix(Axel): **Kiosk-Auto-Scroll stoppt bei Rückkehr in die Normalansicht** — Zyklus hatte nur `start()` ohne Stop (MutationObserver startete nur bei Klasse `kiosk`, entfernte sie aber nie behandelt) → Scroll lief in der Normalansicht weiter. Fix: `running`-Flag + `stop()`; Observer: `if (kiosk) start(); else stop();` — Zyklus (while running), smoothTo (running-check je Frame) und waits brechen sofort ab. Detail- + Admin-Seite. Test: Stop-Marker-Check, 19/19. |
| v1.0.47 | (27.08.) | fix(Axel, Screenshot 22:07): **Abstände vereinheitlicht** — Root Cause: h2 „Module“ + Modul-Karte lagen als Grid-Items IM `.charts`-Grid (Abstand daher Grid-Zeilen-Gap statt h2-Margin). Umbau: Karte+Überschrift aus dem Grid gelöst (`</div>` schließt Charts), Karte 50 % als normaler Block → h2→Karte = 12 px wie „Letzte Stunde“→Charts (h2-Margin, identisch in Kiosk+Normal); Lücke Charts→„Module“ auf 2 px (`#mod-sec-title { margin-top: 2px }` + `.charts .card { margin-bottom: 0 }`). Test: DOM-Reihenfolge- + CSS-Checks, 19/19. |
...
| v1.0.29 | (27.08.) | fix(F60-Live-Bug, von Axel im Screenshot entdeckt): **Detail-Link fehlte nach dem 15-s-Auto-Refresh** — `renderRows` (JS) baute die `inst-rows`-Zeilen ohne Detail-Zelle (7 Zellen in 8-Spalten-Tabelle, das Grafana-Icon rückte optisch in die „Detail“-Spalte; frisch geladen war der Link kurz da, nach dem ersten Refresh weg). Fix: `data-idmap` (nach Leserecht gefiltert!) am `tbody` + `tdDetail` (`.detail-dl`-Auge) in der JS-Render-Funktion + `colSpan=8` im No-Data-Fall + `|default({})` für Render ohne Kontext. Nebenbefund (Rechte-Leak): idmap enthält nur für den Nutzer sichtbare Anlagen. Neue Test-Checks: renderRows-Marker (tdDetail, colSpan 8), idmap-gefiltert. Tests: Suite 14/14 + alle 5 E2E grün (19/19). |
| v1.0.28 | (27.08.) | fix(F60-Folge): Detail-Spalte auf `/monitoring` nutzt eigene Klasse **`.detail-dl`** (blau #8ab4f8) statt `.grafana-dl` — der Rechte-E2E (`monitoring_rechte_e2e`) zählt die Icon-Buttons über `.grafana-dl` (2 erwartet) und brach mit 8, weil die neuen Detail-Links dieselbe Klasse trugen (v1.0.27-Push ohne `*_e2e.py`-Lauf — **Prozess-Lehre: Test-Suite + alle `*_e2e.py` (inkl. `wiki_e2e.py`) IMMER VOR dem Push**, im Skill verankert). Tests: alle 5 E2E grün (admin_filter, badge, monitoring, monitoring_rechte 18/18, wiki 10 Seiten), Suite 14/14. |
| v1.0.27 | (27.08.) | feat(F60): **Anlagen-Detail-Monitoring** `/monitoring/installations/{id}` — 3 Kacheln: SIP-Trunks/Provider **x/x verbunden** (grün=alle verbunden, rot=mind. einer nicht; „keine Provider-Daten“ neutral), **CPU-Last aktuell 1/5/15** (+ Kerne), **Speicher belegt/gesamt + %** (Auslastung wie Grafana: (total−available)/total, >90 % rot). **3 Verlaufs-Graphen „Letzte Stunde“** (SVG Dark Design, handgezeichnet, kein CDN): CPU 1/5/15 in 3 Farben (#4fc3f7/#ffca28/#ab47bc), RAM % (#ffa726), RAM Total (belegt+gesamt gestrichelt). **10-s-Refresh mit Countdown + Kiosk-Modus** (Muster v1.0.20–v1.0.26). Rechte (Axel): Admin oder **can_read** auf die Anlage; **Anlagen-Dropdown nur wenn >1 Anlage sichtbar**; Einstieg: neue Detail-Spalte (Augen-Icon) auf `/monitoring` (`id_by_name` im Kontext); ohne Login Redirect `/`. Daten: Sammler speichert jetzt `system` (load1/5/15, memTotal/memFree/memAvailable, cpuCores — kommen bereits aus GetStats, **kein Modul-Update**); Verlauf via neuer `monitoring.query_system_history` (Flux auf `system`-Measurement, 1-min-Mittel, 60 min, TTL-Cache 15 s, kB→Bytes-Guard); Fallback: InfluxDB-Fehler/leer → Hinweis im Graphen, Kacheln live weiter. Tests: `admin_detail_monitoring_test.py` 76/76, Suite grün. |
| v1.0.26 | (27.08.) | fix(F59/Admin-Monitoring): Auto-Refresh-Takt von 5 s auf **10 s** (Axel: „5 Sekunden ist zu oft“) — gilt für Normal- und Kiosk-Modus: `REFRESH_INTERVAL_S = 10`, initiale Countdown-Werte in Kopf- und Kiosk-Anzeige auf 10 (zählt 10…0, springt zurück). 1-s-Tick + Overlap-Schutz unverändert. Tests: admin_monitoring_test 58/58 (+3: Takt 10, Initialwerte), monitoring_rechte_e2e 18/18. |
| v1.0.25 | (27.08.) | fix(F59/Admin-Monitoring): InfluxDB-Eintrag („konfiguriert/kein Token“) aus der Sammler-Zeile **komplett entfernt** — auch im Normal-Modus (Axel). HTML-Span, JS-`sum-influx`-Update und `.kiosk-hide`-CSS entfallen; Kiosk-Countdown bleibt an dessen Stelle (Sammler-Zeile: „Sammler | Letzter Poll | Aktualisiert in X s“). Backend liefert `influx_configured` weiter (unverändert), nur nicht mehr angezeigt. Tests: admin_monitoring_test 56/56 (Negativ-Checks: kein sum-influx/InfluxDB:/kiosk-hide im Body), monitoring_rechte_e2e 18/18. |
| v1.0.24 | (27.08.) | fix(F59/Admin-Monitoring, Axel-Korrektur): Kiosk-Refresh-Countdown erscheint nicht mehr fest oben links, sondern **an der Stelle des ausgeblendeten InfluxDB-Hinweises** (Sammler-Zeile: „Sammler | Letzter Poll | Aktualisiert in X s“) — in der statline, `display:inline` bei `body.kiosk`. Normalmodus unverändert (Countdown im Seitenkopf, InfluxDB sichtbar). Tests: admin_monitoring_test 55/55, monitoring_rechte_e2e 18/18. |
| v1.0.23 | (27.08.) | feat(F59/Admin-Monitoring, Kiosk-Feinschliff): Im Kiosk-Modus blendet die Seite den Hinweis „InfluxDB: konfiguriert“ aus (`.kiosk-hide`, nur InfluxDB-Eintrag der Sammler-Zeile — Sammler/Letzter Poll bleiben) und zeigt stattdessen den **Refresh-Countdown als festes Element oben links** („Aktualisiert in X s“, `#refresh-countdown-kiosk-num`, JS aktualisiert beide Zähler). Beenden-Button bleibt oben rechts (auto-hide). Tests: admin_monitoring_test 54/54 (+3), monitoring_rechte_e2e 18/18. |
| v1.0.22 | (27.08.) | feat(F59/Admin-Monitoring): Auto-Refresh auf **5-s-Takt** (Axel) mit **Countdown-Anzeige** „Seite aktualisiert sich automatisch in X s“ (zählt pro Sekunde runter, springt nach jedem Refresh zurück auf 5; 1-s-Tick löst den Refresh aus — kein separater Intervall-Timer; Overlap-Schutz `refreshBusy` pausiert den Zähler bei laufendem Fetch, `.finally`-Reset). Tests: admin_monitoring_test 51/51 (+5 Countdown-Checks inkl. „kein 15000 mehr“-Regression), monitoring_rechte_e2e 18/18. |
| v1.0.21 | (27.08.) | fix(F59/Admin-Monitoring): „Vollbild beenden“-Button blendet sich nach 2,5 s ohne Mausbewegung selbst aus (nur bei Mausbewegung/Touch kurz einblenden — Axel-Wunsch, Monitor-Ansicht ungestört): CSS `.fs-hidden` (opacity 0 + pointer-events none, Transition), `fsShow`/Timer, mousemove+touchstart-Wakeup; bei Kiosk-Enter erst sichtbar (Lernphase), ESC/F11/Beenden unverändert. Tests: admin_monitoring_test 47/47 (+5 Auto-Hide-Checks), monitoring_rechte_e2e 18/18. |
| v1.0.20 | (27.08.) | feat(F59/Admin-Monitoring): **Full-Screen/Kiosk-Modus** per Klick (Axel-Wunsch) — Button „Vollbild“ blendet Header/Nav, Footer und Seitenkopf aus (In-Seiten-Kiosk, funktioniert auch am iPhone ohne Fullscreen-API), zusätzlich echter Browser-Fullscreen via Fullscreen-API (Desktop: Adressleiste/Tabs weg; Unterstützung wird erkannt, sonst Fallback). Beenden: schwebender Button oben rechts, ESC oder F11 (fullscreenchange-Listener). Bonus: URL `?kiosk=1` startet direkt im Kiosk (Monitor-Display). Nur `/admin/monitoring`, normale Monitoring-Seite unberührt. Tests: admin_monitoring_test 42/42 (+11 Vollbild-Checks), monitoring_rechte_e2e 18/18. |
| v1.0.19 | (27.08.) | fix(F59-Folge): Admin-Monitoring — Icon der neutralen Karte „Anlagen eingerichtet“ von Akzent-Rot #e94560 auf Blau #8ab4f8 (Axel-Feedback: auf der Seite ist Rot = Fehler-Semantik; die neutrale Gesamt-Kennzahl darf kein rotes Symbol tragen; Fehler-Karten behalten rot/grün). Tests 33/33 + 18/18 grün. |
