...
| Version | Inhalt |
|---|---|
| v0.0.125 | **TV-Kiosk-Modus (obere Leiste auch weg):** Nutzer-Screenshot zeigte: bei `?kiosk` bleibt die obere Grafana-Leiste (Zeitleiste „Last 6 hours", Refresh, Suche) sichtbar und klebt beim Scrollen als Balken mitten im Dashboard. Fix: alle 4 WebApp-Links + Global-Datalink auf `?kiosk=tv` — blendet die Dashboard-Toolbar zusätzlich aus (Live-Deploy Global v7, Datalink verifiziert). „Powered by Grafana"-Badge unten rechts: Grafana-Standard, nicht abschaltbar. |
