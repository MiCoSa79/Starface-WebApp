...
| Version | Inhalt |
|---|---|
| v0.0.141 | **Diagnose „Nicht installiert trotz Installation“:** Admin-Endpoint `module-status-raw` (rohe GetModuleStatus-Antwort je Anlage) + Diagnose-Block auf der Monitoring-Seite; Befund „beide Module rot, obwohl v5 eingerichtet“. Details [[starface-telefonie-monitoring]]. |
| v0.0.140 | **Fix „zu alt“-Meldung:** nennt jetzt v5 (GetModuleStatus-Exporteur) statt v28 — Anlage mit altem Monitoring-Modul zeigt „Update auf v5 erforderlich“. Details [[starface-telefonie-monitoring]]. |
| v0.0.138 | **Modul-Status-Abgleich:** Monitoring-Seite prüft die ausgelieferten „eigenen“ Module (CallBlocker + TelefonieMonitoring) pro Anlage gegen die Anlagen-`ModuleRegistry` (IST vs. SOLL aus `app/modules/*.sfm`): Badges Aktuell / Update verfügbar / **Nicht installiert** / Keine aktive Instanz + Instanz-Status; fehlt das Monitoring-Modul → Hinweis „Monitoring-Modul nicht installiert oder eingerichtet“. Details [[starface-telefonie-monitoring]]. |
| v0.0.135 | **Monitoring-Auto-Refresh (15 s) + Dashboard-Fehlerfixes + Design-Umbau:** (1) Monitoring-Seite: „aktualisiert sich alle 15 s“ war nur Text ohne Code → jetzt `setInterval(refreshMonitoring, 15000)` + `fetch('/api/monitoring/status')` + DOM-Update (tbody `inst-rows`, Status-/Badge-IDs, Provider-Status je Anlage neu gerendert; Grafana-Links nutzen `data-ghref`-Basis mit `kiosk=1&hideLogo`); Browser-Beweis (Headless-Chrome: 1 Call nach Load, +1 nach 17 s, keine JS-Fehler), E2E-Test 15 Referenz-Checks. (2) Admin-Fehlalarm **„Provider getrennt, obwohl verbunden“** behoben: Root Cause = Serienbruch — alte Serien `register=>user@sip…:5060/<Call-ID>` (flüchtiger Suffix!), `register` und `30632@…` hörten auf zu schreiben, ihre letzten `0`-Werte lagen im 6h-Fenster → `last()`+reduce wertete sie als Ausfall. Fix: Ist-Zustands-Panels (Ausfall/Getrennt/Zähler) auf **Frische-Fenster `range(-10m)`** (veraltete Serien = keine Daten ≠ getrennt); „Provider getrennt (gesamt)“ zählt via `sum(down)` (count/reduce liefern auf leerem Input „No data“); Ausfall-Panel per `keep(_value)` auf eine Zahl. (3) Design: „Letzter Poll“-No-data-Bug (`keep(_time)` ohne Zahlfeld → Epoch-ms-Map), „Provider registriert/aktiv“ 1 Wert statt 2 (sum, -10m), Tabellen: `Status`-Spalte „Registriert/Getrennt“ (Mapping, Fließtext statt 1/0), Provider-Breite 480, RAM%-Tabelle im Global-Stil (aggregateWindow→pivot, war None), „Provider-Status je Anlage“ (Provider gesamt/registriert/Alles ok=Ja/Nein), Verläufe RAM+Load nebeneinander, Provider-Status-Y-Achse fix 0..1, Legacy-Serien (`register`, `register=>…`) in Verläufen gefiltert. **Live:** Global v13, Admin v8, Detail v5. E2E 17/17. |
