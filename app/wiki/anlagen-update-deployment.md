...
| RPC-Antwort | `GetAnlagenUpdates` liefert einen **JSON-String** (XML-RPC-String-Typ, z. B. `{"current":"10.0.1.7","count":1,"updates":[{...}]}`) mit description/changelog je Update — die WebApp muss ihn VOLLSTÄNDIG parsen: `_extract_response(raw, max_len=…)` ohne 500-Byte-Kappung im Updates-Pfad (F93, v1.0.91: Kappung schnitt das JSON → „Unerwartete Antwort“) |
