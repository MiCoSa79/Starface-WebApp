...
async function main() {
  ...
  console.log('6) Collapse: Standard eingeklappt + Persistenz');
  check('Initial: alle 3 Tabellen eingeklappt', (await evalJs(`['inst','users','access'].every(k => document.querySelector('[data-wrap="'+k+'"]').classList.contains('collapsed'))`)) === true);
  check('aria-expanded=false initial', (await evalJs(`['inst','users','access'].every(k => document.querySelector('[data-collapse="'+k+'"]').getAttribute('aria-expanded') === 'false')`)) === true);
  await evalJs(`document.querySelector('[data-collapse=inst]').click()`);
  check('Klick: Inst geöffnet', (await evalJs('document.querySelector("[data-wrap=inst]").classList.contains("collapsed")')) === false);
  check('aria-expanded=true', (await evalJs('document.querySelector("[data-collapse=inst]").getAttribute("aria-expanded")')) === 'true');
  ...
  check('Klick: Users geöffnet', (await evalJs('document.querySelector("[data-wrap=users]").classList.contains("collapsed")')) === false);
  check('localStorage Users=1', (await evalJs('localStorage.getItem("sf.admin.collapse.users")')) === '1');
  await send('Page.reload', { ignoreCache: true }); await waitReady();
  await sleep(300);
  check('Nach Reload: Inst offen (gespeichert)', (await evalJs('document.querySelector("[data-wrap=inst]").classList.contains("collapsed")')) === false);
  check('Nach Reload: Users offen (gespeichert)', (await evalJs('document.querySelector("[data-wrap=users]").classList.contains("collapsed")')) === false);
  check('Nach Reload: Access eingeklappt (kein Wert -> Default)', (await evalJs('document.querySelector("[data-wrap=access]").classList.contains("collapsed")')) === true);
  await evalJs(`document.querySelector('[data-collapse=access]').click()`);
  check('Access manuell geöffnet', (await evalJs('document.querySelector("[data-wrap=access]").classList.contains("collapsed")')) === false);
  await evalJs(`document.querySelector('[data-collapse=access]').click()`);
  check('Access wieder eingeklappt', (await evalJs('document.querySelector("[data-wrap=access]").classList.contains("collapsed")')) === true);
