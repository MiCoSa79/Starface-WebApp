...
assert "starface-anlage-detail" in body and "kiosk" in body, "Grafana-UID oder kiosk fehlt im Link"
ok += 1
...
login("admin")
r = c.get("/monitoring")
assert "starface-admin-uebersicht" in r.text and "kiosk" in r.text, "Admin-Übersicht-Link oder kiosk fehlt auf /monitoring"
login("bob")
r = c.get("/monitoring")
body = r.text
...
r = c.get("/admin")
body = r.text
assert "starface-admin-uebersicht" in body and "Grafana Admin-Übersicht öffnen" in body and "kiosk" in body, "Admin-Seite: Admin-Übersicht-Link/kiosk fehlt"
ok += 1
